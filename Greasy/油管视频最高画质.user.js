...
`);
        ...
        const 广告开关映射表 = [{
                cls: '.ns-toggle-ad-force-skip',
                key: '强显跳过按钮'
            },
            {
                cls: '.ns-toggle-ad-mute',
                key: '广告时段静音'
            },
            {
...
`);
    ...
    function 提升视频画质() {
        ...
        const 画质优先级 = 选项.自动画质模式 === '1080p' ? ['hd1080', 'hd720', 'large', 'medium', 'small'] : ['hd2160', 'hd1440', 'hd1080', 'hd720', 'large', 'medium', 'small'];
    ...
    function 处理视频广告() {
        ...
        if (!在广告中) return;
        // [调试] 记录广告检测
        logInfo('[处理视频广告] 检测到广告状态 | classList=' + (播放器 ? Array.from(播放器.classList).filter(function(c) {
            return c.includes('ad')
        }).join(',') : '?') + ' | 强显跳过=' + 选项.强显跳过按钮);
        // ===== 强显跳过按钮 — 让隐藏的跳过按钮可见 =====
        if (选项.强显跳过按钮) {
            强显跳过按钮();
