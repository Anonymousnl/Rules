...
(async function() {
    ...
    const SETTINGS = {
        ...
        useHighestQualityFallback: {
            key: "useHighestQualityFallback",
            def: true
        },
        customSortEnabled: {
            key: "customSortEnabled",
            def: false
        },
        customQualityOrder: {
            key: "customQualityOrder",
            def: null
        },
        takeOverQualityControl: {
            key: "takeOverQualityControl",
            def: false
        },
        unlockUA: {
            key: "unlockUA",
        ...
        preventBackgroundDegrade: {
            key: "preventBackgroundDegrade",
            def: true
        },
        liveAutoRecoverOnVisible: {
            key: "liveAutoRecoverOnVisible",
            def: false
        },
        userLiveQualitySetting: {
            key: "liveQualitySetting",
            def: "最高画质"
        },
        userLiveDecodeSetting: {
            key: "liveDecodeSetting",
            def: "默认"
        },
        userVideoDecodeSetting: {
            key: "videoDecodeSetting",
            def: "默认"
        },
        decodeSettingEnabled: {
            key: "decodeSettingEnabled",
            def: false
        },
        devModeEnabled: {
            key: "devModeEnabled",
        ...
        devModeVipStatus: {
            key: "devModeVipStatus",
            def: "默认"
        },
        devModeNoLoginStatus: {
            key: "devModeNoLoginStatus",
            def: false
        },
        preserveTouchPoints: {
            key: "preserveTouchPoints",
            def: false
        },
        devModeAudioRetries: {
            key: "devModeAudioRetries",
            def: 2
        },
        devModeAudioDelay: {
            key: "devModeAudioDelay",
            def: 5000
        },
        devDoubleCheckDelay: {
            key: "devDoubleCheckDelay",
            def: 10000
        },
        devAllowFreeVipQualities: {
            key: "devAllowFreeVipQualities",
            def: false
        },
        injectQualityButton: {
            key: "injectQualityButton",
            def: true
        },
        qualityDoubleCheck: {
            key: "qualityDoubleCheck",
            def: true
        },
        liveQualityDoubleCheck: {
            key: "liveQualityDoubleCheck",
            def: true
        },
        liveQualityPollingEnabled: {
            key: "liveQualityPollingEnabled",
            def: false
        },
        livePollingInterval: {
            key: "livePollingInterval",
            def: 60
        },
        liveKeepAliveEnabled: {
            key: "liveKeepAliveEnabled",
            def: false
        },
        liveKeepAliveInterval: {
            key: "liveKeepAliveInterval",
            def: 60
        },
        autoWidescreen: {
            key: "autoWidescreen",
...
`);
    ...
    function normalizeCustomOrder(order) {
        ...
        if (Array.isArray(order)) {
            order.forEach(q => {
                if (known.has(q) && !seen.has(q)) {
                    result.push(q);
                    seen.add(q);
                }
            });
        }
        DEFAULT_CUSTOM_ORDER.forEach(q => {
            if (!seen.has(q)) {
                result.push(q);
                seen.add(q);
            }
...
`;
    ...
    function setupCustomGridDrag(panel) {
        ...
        function clearPress() {
            ...
            if (pressEl) {
                pressEl.classList.remove("cq-press");
                pressEl = null;
            }
        }
        ...
        function pointerRel() {
            const gridRect = grid.getBoundingClientRect();
            return {
                x: lastClientX - gridRect.left,
                y: lastClientY - gridRect.top
            };
        }
        ...
        grid.addEventListener("contextmenu", (e) => {
            if (activated || pressTimer) e.preventDefault();
        });
    }
    ...
    async function selectVideoQuality() {
        ...
        function cleanQuality(q) {
            if (!q) return "";
            let result = q;
            CLEAN_KEYWORDS.forEach(k => {
                result = result.replace(new RegExp(k, 'g'), '');
            });
            return result.trim();
        }
        ...
        if (state.isLivePage) {
            ...
            const vipCheckObserver = new MutationObserver((mutations, observer) => {
                ...
                if (headerAvatar) {
                    ...
                    fetch("https://api.bilibili.com/x/vip/web/user/info", {
                            credentials: "include"
                        })
                        .then(r => r.json())
                        .then(d => {
