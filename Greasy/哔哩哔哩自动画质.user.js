...
// @downloadURL https://github.com/Anonymousnl/Rules/raw/master/Greasy/%E5%93%94%E5%93%A9%E5%93%94%E5%93%A9%E8%87%AA%E5%8A%A8%E7%94%BB%E8%B4%A8.user.js
// @updateURL   https://github.com/Anonymousnl/Rules/raw/master/Greasy/%E5%93%94%E5%93%A9%E5%93%94%E5%93%A9%E8%87%AA%E5%8A%A8%E7%94%BB%E8%B4%A8.user.js
// ==/UserScript==
(async function() {
    ...
    try {
        ...
        if (state.unlockUA && state.devModeEnabled && state.preserveTouchPoints) {
            ...
            if (pointerType.isMouseDevice && !pointerType.isTouchDevice) {
                ...
                console.log(`[解锁设置] 保留 maxTouchPoints 原值: ${navigator.maxTouchPoints}，原因: ` + `${pointerType.isTouchDevice ? "检测到触控设备" : "无精确鼠标指针"}`);
            }
        }
    } catch (error) {
...
`);
    ...
    const PANEL_IDS = ["bilibili-quality-selector", "bilibili-live-quality-selector", "bilibili-dev-settings", "bilibili-unlock-settings", "bilibili-decode-settings"];
    const QUALITY_ORDER = ["8K", "杜比视界", "HDR", "4K", "1080P 高码率", "1080P 60帧", "1080P 高清", "720P 60帧", "720P", "480P", "360P", "默认"];
...
<path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
    ...
    function renderGithubLink() {
        return `<a href="https://github.com/Anonymousnl/Rules/tree/master/Greasy" target="_blank" class="github-link">${GITHUB_SVG}</a>`;
    }
    ...
    function updateQualityButtons(panel) {
        ...
        if (state.isLoading) {
            ...
            if (state.vipStatusChecked) {
                statusBar.textContent = state.isVipUser ? "您是大会员用户，可正常使用所有选项。" : "您不是大会员用户，部分会员选项不可用。";
        ...
        Utils.queryAll(".quality-button", panel).forEach(button => {
            ...
            if (
                (section.getAttribute("data-section") === "primary" && button.getAttribute("data-quality") === state.userQualitySetting) || (section.getAttribute("data-section") === "backup" && button.getAttribute("data-quality") === state.userBackupQualitySetting)) {
...
`;
        panel.addEventListener("click", function(e) {
            ...
            if (target.classList.contains("quality-tab") && !state.isLoading) {
                ...
                Utils.queryAll(".quality-tab", panel).forEach(tab => tab.classList.toggle("active", tab.getAttribute("data-tab") === tabName));
                const tabs = panel.querySelector('.quality-tabs');
                if (tabs) tabs.setAttribute('data-active', tabName);
                // 切换可见的画质区域
                Utils.queryAll(".quality-section", panel).forEach(section => section.classList.toggle("active", section.getAttribute("data-section") === tabName));
                // 切换后同步状态与高亮
    ...
    async function selectVideoQuality() {
        ...
        const qualityItems = Array.from(qualityMenu.querySelectorAll(".bpx-player-ctrl-quality-menu-item"));
        ...
        function cleanQuality(q) {
            return q ? q.replace(/大会员|限免中/g, '').trim() : "";
        }
        if (state.userQualitySetting === "最高画质") {
            ...
            if (state.isVipUser || hasFreeVip) {
                ...
                if (!targetQuality && state.useHighestQualityFallback) targetQuality = availableQualities.find(q => !q.isVipOnly);
            ...
            if (!targetQuality) {
                ...
                targetQuality = availableQualities.find(q => cleanQuality(q.name).includes(state.userBackupQualitySetting));
                if (!targetQuality && state.useHighestQualityFallback) targetQuality = state.isVipUser ? availableQualities[0] : availableQualities.find(q => !q.isVipOnly);
                if (!targetQuality && !state.useHighestQualityFallback) {
                    console.log("[画质设置] 未找到备选画质，保持当前画质");
                    await setAudioQuality();
                    return;
...
`;
    ...
    function initPlayerScripts() {
        ...
        if (state.isLivePage) {
            ...
            if (playerControls) {
                window.playerControlsObserver.observe(playerControls, {
                    childList: true,
                    subtree: true
                });
            }
    ...
    window.addEventListener("DOMContentLoaded", initPlayerScripts, {
        once: true
    });
