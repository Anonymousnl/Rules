...
(function() {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.tag-item_3Z1mA, .tag-common_8ztfL, [class*="tag-"]')) {
            ...
            setTimeout(() => {
                ...
                obs.observe(document.querySelector('#content_left') || document.body, {
                    childList: true,
                    subtree: true
                });
    ...
    window.AutoPagination = {
        ...
        enable() {
            ...
            console.log('[自动翻页] 已启用, 起始pn:', this.currentPn);
        },
        disable() {
            this.enabled = false;
            if (this.scrollHandler) window.removeEventListener('scroll', this.scrollHandler);
            console.log('[自动翻页] 已禁用');
        },
        bindScrollEvent() {
            if (this.scrollHandler) window.removeEventListener('scroll', this.scrollHandler);
        ...
        checkAndLoad() {
            ...
            if (distanceToBottom < 500) {
                console.log('[自动翻页] 触发加载，距离底部:', Math.round(distanceToBottom), 'px');
                this.loadNextPage();
            }
        },
        loadNextPage() {
            ...
            console.log('[自动翻页] 加载第', this.currentPage + 1, '页, pn:', this.currentPn, ', URL:', nextUrl);
            GM_xmlhttpRequest({
                method: 'GET',
                url: nextUrl,
                onload: (response) => {
                    try {
                        ...
                        this.currentPage++;
                        console.log('[自动翻页] ✅ 成功加载第', this.currentPage, '页，新增', addedCount, '条结果');
                        this.showLoading(false, `✅ 已加载第 ${this.currentPage} 页 (${addedCount}条)`);
                    } catch (e) {
                        console.error('[自动翻页] 加载失败:', e);
                        this.showLoading(false, '❌ 加载失败');
                    }
                    this.isLoading = false;
                },
                onerror: () => {
                    console.error('[自动翻页] 网络错误');
    ...
    const commonStyles =
        ...
        // 针对双列模式下"内容过窄"的精准修复
        // ==============================================
        // 强制 c-span24 (通用的全宽容器) 占满 100%，修复窄内容
        ...
        //  播报/暂停组件容器 - 强制横向排列 (flex-direction: row)，不换行
        ...
        // 修复百科图片变成巨大正方形的问题
        // ==========================================================================
        // 覆盖百度原生的 padding-bottom: 100% 规则，取消正方形强制比例
        ...
        // 针对 c-img-s 类的补充修复 (以防没有 site-img 包裹的情况)
        ...
        // ==========================================================================
        //  覆盖百度原生的 padding-bottom 规则（只针对顶部图片区域）
        ...
        //  百科左侧缩略图（c-img-s in c-span3）- 强制 120px x 100px
        ...
        // 百科右侧大图（c-img3 in c-span9）- 强制 140px x 100px
        ...
        // 确保底部来源图标保持16x16（明确排除site-img区域的c-img-s）
        ...
        // 百科/关联卡片图片“长条化”彻底矫正
        // ==========================================================================
        // 锁定 lemma-item 容器，强制使用 Flex 布局并允许内容撑开高度
        ...
        '}' +
        // 彻底重置左侧图片包装层 (c-span2)，干掉所有浮动和百分比限制
