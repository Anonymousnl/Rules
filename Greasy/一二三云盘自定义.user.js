...
(function() {
    ...
    class PanApiClient {
        ...
        async getFileInfo(idList) {
            const fileIdList = idList.map(fileId => ({
                fileId
            }));
            const data = await this.sendRequest("POST", "/b/api/file/info", {}, JSON.stringify({
                fileIdList
            }));
            return {
                data: {
    ...
    function createSettingsPanel() {
        ...
        const settings = [{
            ...
            {
                key: '秒传功能',
                value: FastLinkConfig.enabled,
                comment: '启用秒传链接生成和保存功能'
            },
            {
                key: '用户名',
                value: user.name,
