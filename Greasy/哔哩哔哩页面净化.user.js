...
`));
	...
	function useEventListener(...args) {
		...
		const stopWatch = e$1.watch(
			() => [unrefElement(target), e$1.toValue(options)],
			([el, options2]) => {
				cleanup();
				if(!el) return;
	...
	function useMediaQuery(query, options = {}) {
		...
		const stopWatch = e$1.watchEffect(() => {
			...
			cleanup();
			mediaQuery = window2.matchMedia(e$1.toValue(query));
			...
			get(target2, prop, rec) {
				...
				if(!(prop in refs)) {
					if(/[+_-]/.test(prop)) {
						...
						refs[prop] = e$1.computed(() => keys2.every((key) => e$1.toValue(proxy[key])));
					} else {
	...
	function useWindowSize(options = {}) {
		...
		if(window2 && type === "visual" && window2.visualViewport) {
			useEventListener(window2.visualViewport, "resize", update, {
				passive: true
			});
		}
