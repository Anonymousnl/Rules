	...
	class DynamicFilterDynamic {
		...
		async check(mode) {
			...
			if(settings.enableDebugFilter) {
				dyns.forEach((v2) => {
					debugFilter(
						[`DynamicFilterDynamic`, `uploader: ${selectorFns$7.uploader(v2)}`, `title: ${selectorFns$7.title(v2)}`, `duration: ${selectorFns$7.duration(v2)}`, `content: ${selectorFns$7.content(v2)}`].join("\n"));
				});
			}
			const blackPairs = [];
	...
	class DynamicFilterSpace {
		...
		async check(mode) {
			...
			if(settings.enableDebugFilter) {
				dyns.forEach((v2) => {
					debugFilter(
						[`DynamicFilterSpace`, `title: ${selectorFns$6.title(v2)}`, `duration: ${selectorFns$6.duration(v2)}`, `content: ${selectorFns$6.content(v2)}`].join("\n"));
				});
			}
			const blackPairs = [];
