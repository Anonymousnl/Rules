...
// @grant       GM_setValue
// @grant       unsafeWindow
// @require     https://registry.npmmirror.com/vue/3.5.16/files/dist/vue.global.prod.js
...
`));
		...
		setup(__props) {
			...
			e$1.watch(selectedOption, (newSelected) => {
				...
				try {
					...
					BiliCleanerStorage.set(item.id, newSelected.value);
				} catch (err) {
	...
	class VideoFilterChannel {
		...
		observe() {
			waitForEle(document, ".channel-page__body", (node) => {
...
`);
		...
		noStyle: true,
		enableFn: () => {
			ShadowInstance.addShadowStyle("bili-rich-text", "video-page-hide-jump-link-search-word", `#contents a[href*="search.bilibili.com"] {color: inherit !important;}
