...
`));
	...
	const isPageChannelNext = () => ans === "channelNext";
	...
	class DynamicFilterSpace {
		...
		async check(mode) {
			...
			if(settings.enableDebugFilter) {
				dyns.forEach((v2) => {
					debugFilter(
						[`DynamicFilterSpace`, `title: ${selectorFns$7.title(v2)}`, `duration: ${selectorFns$7.duration(v2)}`, `content: ${selectorFns$7.content(v2)}`, `shortVideo: ${selectorFns$7.dynVideo(v2)}`, `playback: ${selectorFns$7.playback(v2)}`].join("\n"));
				});
			}
			const blackPairs = [];
	...
	const mainFilter$7 = new DynamicFilterSpace();
	...
	class VideoFilterChannel {
		...
		init() {
			this.videoBvidFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.black.bvid.valueKey, []));
			this.videoDurationFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.black.duration.valueKey, 0));
			this.videoTitleFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.black.title.valueKey, []));
			this.videoPubdateFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.black.pubdate.valueKey, 0));
			this.videoUploaderFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.black.uploader.valueKey, []));
			this.videoUploaderKeywordFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.black.uploaderKeyword.valueKey, []));
			this.videoUploaderWhiteFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.white.uploader.valueKey, []));
			this.videoTitleWhiteFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.white.title.valueKey, []));
		}
		async check(mode) {
			if(!this.target) {
				return;
			...
			if(settings.enableDebugFilter) {
				videos.forEach((v2) => {
					debugFilter(
						[`VideoFilterChannel`, `bvid: ${selectorFns$6.bvid(v2)}`, `duration: ${selectorFns$6.duration(v2)}`, `title: ${selectorFns$6.title(v2)}`, `uploader: ${selectorFns$6.uploader(v2)}`, `pubdate: ${selectorFns$6.pubdate(v2)}`].join("\n"));
				});
			}
			const blackPairs = [];
			this.videoBvidFilter.isEnable && blackPairs.push([this.videoBvidFilter, selectorFns$6.bvid]);
	...
	const mainFilter$6 = new VideoFilterChannel();
	...
	const videoFilterChannelGroups = [{
		...
		items: [{
			...
			saveFn: async () => {
				mainFilter$6.videoUploaderFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.black.uploader.valueKey, []));
				mainFilter$6.checkFull();
			}
		}, {
			type: "switch",
			...
			editorDescription: ["每行一个关键词或正则，不区分大小写", "请勿使用过于激进的关键词或正则", "正则默认 ius 模式，无需 flag，语法：/abc|\\d+/"],
			saveFn: async () => {
				mainFilter$6.videoUploaderKeywordFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.black.uploaderKeyword.valueKey, []));
				mainFilter$6.checkFull();
			}
		}]
	}, {
		...
		items: [{
			...
			editorDescription: ["每行一个关键词或正则，不区分大小写", "请勿使用过于激进的关键词或正则", "正则默认 ius 模式，无需 flag，语法：/abc|\\d+/"],
			saveFn: async () => {
				mainFilter$6.videoTitleFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.black.title.valueKey, []));
				mainFilter$6.checkFull();
			}
		}]
	}, {
		...
		items: [{
			...
			saveFn: async () => {
				mainFilter$6.videoBvidFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.black.bvid.valueKey, []));
				mainFilter$6.checkFull();
			}
		}]
	}, {
		...
		items: [{
			...
			editorDescription: ["每行一个UP主昵称，保存时自动去重"],
			saveFn: async () => {
				mainFilter$6.videoUploaderWhiteFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.white.uploader.valueKey, []));
				mainFilter$6.checkFull();
			}
		}, {
			type: "switch",
			...
			editorDescription: ["每行一个关键词或正则，不区分大小写", "请勿使用过于激进的关键词或正则", "正则默认 ius 模式，无需 flag，语法：/abc|\\d+/"],
			saveFn: async () => {
				mainFilter$6.videoTitleWhiteFilter.setParam(BiliCleanerStorage.get(GM_KEYS$6.white.title.valueKey, []));
				mainFilter$6.checkFull();
			}
		}]
	}];
	const videoFilterChannelHandler = (target) => {
		...
		if(target.closest(".bili-video-card__info--owner")) {
			...
			if(uploader) {
				if(mainFilter$6.videoUploaderFilter.isEnable) {
					menus.push({
						name: `屏蔽UP主：${uploader}`,
						fn: async () => {
							try {
				...
				if(mainFilter$6.videoUploaderWhiteFilter.isEnable) {
					menus.push({
						name: `将UP主加入白名单`,
						fn: async () => {
							try {
								...
								mainFilter$6.checkFull();
								const arr = BiliCleanerStorage.get(GM_KEYS$6.white.uploader.valueKey, []);
								arr.unshift(uploader);
								BiliCleanerStorage.set(GM_KEYS$6.white.uploader.valueKey, orderedUniq(arr));
							} catch (err) {
			...
			if(spaceUrl && (mainFilter$6.videoUploaderFilter.isEnable || mainFilter$6.videoUploaderWhiteFilter.isEnable)) {
				menus.push({
					name: `复制主页链接`,
					fn: async () => {
					...
					menus.push({
						...
						fn: async () => {
							try {
								...
								mainFilter$6.checkFull();
								const arr = BiliCleanerStorage.get(GM_KEYS$6.black.bvid.valueKey, []);
								arr.unshift(bvid);
								BiliCleanerStorage.set(GM_KEYS$6.black.bvid.valueKey, orderedUniq(arr));
							} catch (err) {
	...
	class VideoFilterChannelNext {
		...
		async check(mode) {
			...
			if(settings.enableDebugFilter) {
				videos.forEach((v2) => {
					debugFilter(
						[`VideoFilterChannelNext`, `bvid: ${selectorFns$5.bvid(v2)}`, `duration: ${selectorFns$5.duration(v2)}`, `title: ${selectorFns$5.title(v2)}`, `uploader: ${selectorFns$5.uploader(v2)}`, `pubdate: ${selectorFns$5.pubdate(v2)}`].join("\n"));
				});
			}
			const blackPairs = [];
			...
			const blackCnt = await coreCheck(videos, true, blackPairs, whitePairs);
			const time = (performance.now() - timer).toFixed(1);
			debugFilter(`VideoFilterChannelNext hide ${blackCnt} in ${videos.length} videos, mode=${mode}, time=${time}`);
		}
		checkFull() {
			this.check("full").then().catch((err) => {
		...
		observe() {
			waitForEle(document, ".channel-page__body", (node) => {
				...
				debugFilter("VideoFilterChannelNext target appear");
				this.target = ele;
				this.checkFull();
				new MutationObserver(() => {
					this.checkIncr();
	...
	const mainFilter$5 = new VideoFilterChannelNext();
	const videoFilterChannelNextEntry = async () => {
		mainFilter$5.init();
		mainFilter$5.observe();
	};
	const videoFilterChannelNextGroups = [{
		name: "时长过滤",
		items: [{
			type: "switch",
	...
	const videoFilterChannelNextHandler = (target) => {
		var _a, _b, _c, _d, _e, _f, _g;
...
https://${domain}/${avbv}` : `https://${domain}/${avbv}`;
		...
		setup(__props) {
			...
			return (_ctx, _cache) => {
				return e$1.unref(sideBtnStore).isShow ? (e$1.openBlock(), e$1.createElementBlock("div", {
					...
					e$1.unref(isPageVideo)() || e$1.unref(isPageChannel)() || e$1.unref(isPageChannelNext)() || e$1.unref(isPageHomepage)() || e$1.unref(isPagePlaylist)() || e$1.unref(isPageSearch)() || e$1.unref(isPagePopular)() ? (e$1.openBlock(), e$1.createElementBlock("div", {
						key: 2,
						class: "mt-1 hidden h-10 w-10 cursor-pointer items-center justify-center rounded-lg border border-gray-200 bg-white transition-colors hover:border-none hover:bg-[#00AEEC] hover:text-white group-hover:flex",
	...
	const menu = () => {
		...
		if(isPageHomepage() || isPageVideo() || isPagePlaylist() || isPagePopular() || isPageChannel() || isPageChannelNext() || isPageSearch() || isPageSpace()) {
