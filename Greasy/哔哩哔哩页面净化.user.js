...
// @downloadURL https://github.com/Anonymousnl/Rules/raw/refs/heads/master/Greasy/%E5%93%94%E5%93%A9%E5%93%94%E5%93%A9%E9%A1%B5%E9%9D%A2%E5%87%80%E5%8C%96.user.js
// @updateURL   https://github.com/Anonymousnl/Rules/raw/refs/heads/master/Greasy/%E5%93%94%E5%93%A9%E5%93%94%E5%93%A9%E9%A1%B5%E9%9D%A2%E5%87%80%E5%8C%96.user.js
// ==/UserScript==
(function(e$1) {
...
`));
			...
			get(target2, prop, rec) {
				...
				if(!(prop in refs)) {
					if(/[+_-]/.test(prop)) {
						...
						refs[prop] = e$1.computed(() => keys2.every((key) => toValue(proxy[key])));
					} else {
						refs[prop] = e$1.ref(false);
					}
				}
	...
	const selectorFns$7 = {
		root: {
			...
			callBot: (comment) => {
				const members = Array.from(comment.querySelectorAll(".root-reply-container .reply-content .jump-link.user"));
				return members.some((v2) => botsSet.has(v2.textContent.replace("@", "")));
			},
			callUser: (comment) => {
				var _a, _b;
		...
		sub: {
			...
			callBot: (comment) => {
				const members = Array.from(comment.querySelectorAll(".reply-content .jump-link.user"));
				return members.some((v2) => botsSet.has(v2.textContent.replace("@", "")));
			},
			callUser: (comment) => {
				var _a, _b, _c, _d, _e;
	...
	class CommentFilterLegacy {
		...
		async check(mode) {
			...
			if(settings.enableDebugFilter) {
				rootComments.forEach((v2) => {
					debugFilter(
						[`CommentFilterLegacy rootComments`, `username: ${selectorFns$7.root.username(v2)}`, `content: ${selectorFns$7.root.content(v2)}`, `callUser: ${selectorFns$7.root.callUser(v2)}`, `callUserOnly: ${selectorFns$7.root.callUserOnly(v2)}`, `level: ${selectorFns$7.root.level(v2)}`, `isUp: ${selectorFns$7.root.isUp(v2)}`, `isPin: ${selectorFns$7.root.isPin(v2)}`, `isNote: ${selectorFns$7.root.isNote(v2)}`, `isLink: ${selectorFns$7.root.isLink(v2)}`].join("\n"));
				});
				subComments.forEach((v2) => {
					debugFilter(
						[`CommentFilterLegacy subComments`, `username: ${selectorFns$7.sub.username(v2)}`, `content: ${selectorFns$7.sub.content(v2)}`, `callUser: ${selectorFns$7.sub.callUser(v2)}`, `callUserOnly: ${selectorFns$7.sub.callUserOnly(v2)}`, `level: ${selectorFns$7.sub.level(v2)}`, `isUp: ${selectorFns$7.sub.isUp(v2)}`, `isLink: ${selectorFns$7.sub.isLink(v2)}`].join("\n"));
				});
			}
			...
			const time = (performance.now() - timer).toFixed(1);
			debugFilter(`CommentFilterLegacy hide ${rootBlackCnt} in ${rootComments.length} root, ${subBlackCnt} in ${subComments.length} sub, mode=${mode}, time=${time}`);
		}
		checkFull() {
		...
		observe() {
			waitForEle(document, "#app", (node) => {
				...
				if(ele) {
					debugFilter("CommentFilterLegacy target appear");
					this.target = ele;
					this.checkFull();
	...
	const mainFilter$7 = new CommentFilterLegacy();
	const commentFilterLegacyEntry = async () => {
		mainFilter$7.init();
		mainFilter$7.observe();
	};
	const commentFilterLegacyGroups = [{
		name: "评论用户过滤",
		items: [{
			type: "switch",
	...
	const commentFilterLegacyHandler = (target) => {
		var _a, _b;
		if(!isPageSpace()) {
			return [];
		}
	...
	class DynamicFilterDynamic {
		...
		async check(mode) {
			...
			if(settings.enableDebugFilter) {
				dyns.forEach((v2) => {
					debugFilter(
						[`DynamicFilterDynamic`, `uploader: ${selectorFns$6.uploader(v2)}`, `title: ${selectorFns$6.title(v2)}`, `duration: ${selectorFns$6.duration(v2)}`, `content: ${selectorFns$6.content(v2)}`].join("\n"));
				});
			}
			const blackPairs = [];
...
https://www.bilibili.com/video/${avbv}` : `https://www.bilibili.com/video/${avbv}`;
	...
	const searchStyle = 'html[hide-search-page-search-sticky-header] .search-sticky-header{display:none!important}html[hide-search-page-ad] .video-list.row>div:has([href*="cm.bilibili.com"],.bili-video-card__info--ad,.bili-video-card__info--ad-creative){display:none!important}html[hide-search-page-bangumi-pgc-list] .bangumi-pgc-list,html[hide-search-page-activity-game-list] .activity-game-list{display:none!important}html[hide-search-page-live-room-result] .video-list>div:has([href*="live.bilibili.com"]){display:none!important}html[hide-search-page-danmaku-count] .bili-video-card .bili-video-card__stats--left .bili-video-card__stats--item:nth-child(2){display:none!important}html[hide-search-page-date] .bili-video-card .bili-video-card__info--date,html[hide-search-page-bili-watch-later] .bili-video-card .bili-watch-later{display:none!important}html[hide-search-page-customer-service] .side-buttons div:has(>a[href*=customer-service]){display:none!important}html[hide-search-page-btn-to-top] .side-buttons .btn-to-top-wrap{display:none!important}';
