...
`));
				...
				try {
					for(const option of options) {
						if(option.value === newSelected.value) {
							if(option.fn) {
								(_a = option.fn()) == null ? void 0 : _a.then().catch();
							}
	...
	const commentFilterCommonGroups = [{
		...
		items: [{
			...
			editorDescription: ["每行一个关键词或正则，不区分大小写", "请勿使用过于激进的关键词或正则", "正则默认 ius 模式，无需 flag，语法：/abc|\\d+/"],
			saveFn: async () => {
...
--bili-comment-hover-more-display: 1 !important;
			...
			ShadowInstance.addShadowStyle("bili-comment-action-buttons-renderer", "video-page-hide-root-reply-dislike-reply-btn", `#dislike button, #reply button, #more button {
...
transition: opacity 0.2s 0.3s;
		...
		noStyle: true,
		enableFn: () => {
			ShadowInstance.addShadowStyle("bili-rich-text", "video-page-hide-emoji-large", `#contents img:is(.emoji-large, [style^="width:50px"]) {display: none !important;}`);
		},
		disableFn: () => {
		...
		noStyle: true,
		enableFn: () => {
			ShadowInstance.addShadowStyle("bili-rich-text", "video-page-hide-emoji-large-zoom", `#contents img:is(.emoji-large, [style^="width:50px"]) {zoom: 0.5 !important;}`);
		},
		disableFn: () => {
