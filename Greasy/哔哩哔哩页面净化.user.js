...
// @grant       GM_setValue
// @grant       unsafeWindow
// @require     https://registry.npmmirror.com/vue/3.5.11/files/dist/vue.global.prod.js
...
`));
	...
	function tryOnMounted(fn, sync = true, target) {
		...
		if(instance) e$1.onMounted(fn, target);
		else if(sync) fn();
		else e$1.nextTick(fn);
	}
	...
	function useDraggable(target, options = {}) {
		...
		const position = e$1.ref(
			(_a = toValue(initialValue)) != null ? _a : {
				x: 0,
				y: 0
			});
		...
		const handleEvent = (e2) => {
			if(toValue(preventDefault2)) e2.preventDefault();
			if(toValue(stopPropagation)) e2.stopPropagation();
		};
		const start = (e2) => {
			var _a2;
			if(!toValue(buttons).includes(e2.button)) return;
			if(toValue(options.disabled) || !filterEvent(e2)) return;
			if(toValue(exact) && e2.target !== toValue(target)) return;
		...
		setup(__props, {
			...
			e$1.watch([x, y2], ([newX, newY]) => {
				...
				if(newY + height.value > innerHeight) {
					y2.value = innerHeight - height.value;
				}
			...
			return (_ctx, _cache) => {
				return e$1.openBlock(), e$1.createElementBlock("div", {
					...
					style: e$1.normalizeStyle([{
							width: _ctx.widthPercent + "vw",
							height: _ctx.heightPercent + "vh",
							minWidth: _ctx.minWidth + "px",
	...
	const _sfc_main$b = /* @__PURE__ */ e$1.defineComponent({
		...
		setup(__props) {
			...
			const updateData = () => {
				editorData.value = BiliCleanerStorage.get(item.id, []).join("\n") + "\n";
			};
			const saveData = () => {
				try {
	...
	const _sfc_main$9 = /* @__PURE__ */ e$1.defineComponent({
		...
		setup(__props) {
			...
			e$1.watch(currValue, (newValue, oldValue) => {
				var _a;
				try {
	...
	const selectorFns$9 = {
		...
		sub: {
			...
			callUser: (comment) => {
				...
				return (_f = (_e = (_d = (_c = (_b = (_a = comment.__data) == null ? void 0 : _a.content) == null ? void 0 : _b.message) == null ? void 0 : _c.trim()) == null ? void 0 : _d.replace(/^回复\s?@[^@\s]+\s?:/, "")) == null ? void 0 : _e.match(/@[^@\s]+/)) == null ? void 0 : _f[0].replace("@", "").trim();
			},
			callUserOnly: (comment) => {
				var _a, _b, _c, _d, _e;
				return ((_e = (_d = (_c = (_b = (_a = comment.__data) == null ? void 0 : _a.content) == null ? void 0 : _b.message) == null ? void 0 : _c.trim()) == null ? void 0 : _d.replace(/^回复\s?@[^@\s]+\s?:/, "")) == null ? void 0 : _e.replace(/@[^@\s]+/g, " ").trim()) === "";
	...
	class CommentFilterDynamic {
		...
		init() {
			const bots = [
				...
				// 605801219 @AI工具集
				"机器工具人", "有趣的程序员", "AI视频小助理", "AI视频小助理总结一下", "AI笔记侠", "AI视频助手", "哔哩哔理点赞姬", "课代表猫", "AI课代表呀", "木几萌Moe", "星崽丨StarZai", "AI沈阳美食家", "AI头脑风暴", "GPT_5", "Juice_AI", "AI全文总结", "AI视频总结", "AI总结视频", "AI工具集"
			];
		...
		async checkRoot(mode) {
			...
			let revertAll = false;
			if(!(this.commentUsernameFilter.isEnable || this.commentContentFilter.isEnable || this.commentLevelFilter.isEnable || this.commentBotFilter.isEnable || this.commentCallBotFilter.isEnable || this.commentCallUserFilter.isEnable || this.commentCallUserOnlyFilter.isEnable)) {
				revertAll = true;
			}
			let rootComments = [];
			...
			if(settings.enableDebugFilter) {
				rootComments.forEach((v2) => {
					debugFilter(
						[`CommentFilterDynamic rootComments`, `username: ${selectorFns$9.root.username(v2)}`, `content: ${selectorFns$9.root.content(v2)}`, `callUser: ${selectorFns$9.root.callUser(v2)}`, `callUserOnly: ${selectorFns$9.root.callUserOnly(v2)}`, `level: ${selectorFns$9.root.level(v2)}`, `isUp: ${selectorFns$9.root.isUp(v2)}`, `isPin: ${selectorFns$9.root.isPin(v2)}`, `isNote: ${selectorFns$9.root.isNote(v2)}`, `isLink: ${selectorFns$9.root.isLink(v2)}`].join("\n"));
				});
			}
			if(isRootWhite$2 || revertAll) {
			...
			this.commentCallBotFilter.isEnable && blackPairs.push([this.commentCallBotFilter, selectorFns$9.root.callUser]);
			...
			debugFilter(`CommentFilterDynamic hide ${rootBlackCnt} in ${rootComments.length} root comments, mode=${mode}, time=${time}`);
		}
		/**
		 * 检测二级评论
		 * @param mode full全量，incr增量
		...
		async checkSub(mode) {
			...
			if(settings.enableDebugFilter) {
				subComments.forEach((v2) => {
					debugFilter(
						[`CommentFilterDynamic subComments`, `username: ${selectorFns$9.sub.username(v2)}`, `content: ${selectorFns$9.sub.content(v2)}`, `callUser: ${selectorFns$9.sub.callUser(v2)}`, `callUserOnly: ${selectorFns$9.sub.callUserOnly(v2)}`, `level: ${selectorFns$9.sub.level(v2)}`, `isUp: ${selectorFns$9.sub.isUp(v2)}`, `isLink: ${selectorFns$9.sub.isLink(v2)}`].join("\n"));
				});
			}
			if(isSubWhite$2 || revertAll) {
			...
			this.commentCallBotFilter.isEnable && blackPairs.push([this.commentCallBotFilter, selectorFns$9.sub.callUser]);
			...
			const time = (performance.now() - timer).toFixed(1);
			debugFilter(`CommentFilterDynamic hide ${subBlackCnt} in ${subComments.length} sub comments, mode=${mode}, time=${time}`);
		}
		check(mode) {
	...
	const mainFilter$9 = new CommentFilterDynamic();
	const commentFilterDynamicEntry = async () => {
		mainFilter$9.init();
		mainFilter$9.observe();
	};
	const commentFilterDynamicGroups = [{
		name: "评论用户过滤",
		items: [{
			type: "switch",
		...
		items: [{
			...
			editorDescription: ["每行一个关键词或正则，不区分大小写", "请勿使用过于激进的关键词或正则", "正则默认iu模式，无需flag，语法：/abc|\\d+/"],
			saveFn: async () => {
	...
	const commentFilterDynamicHandler = (target) => {
		var _a, _b;
		if(!isPageDynamic()) {
			return [];
		}
	...
	const selectorFns$8 = {
		root: {
			...
			content: (comment) => {
				var _a, _b;
				return (_b = (_a = comment.querySelector(".root-reply-container .reply-content")) == null ? void 0 : _a.textContent) == null ? void 0 : _b.trim().replace(/@[^@\s]+/g, " ").trim();
			},
			callUser: (comment) => {
				var _a, _b;
			...
			callUserOnly: (comment) => {
				var _a, _b;
				return ((_b = (_a = comment.querySelector(".root-reply-container .reply-content")) == null ? void 0 : _a.textContent) == null ? void 0 : _b.replace(/@[^@\s]+/g, " ").trim()) === "";
			},
			level: (comment) => {
				var _a, _b;
				const c2 = (_a = comment.querySelector(".root-reply-container .user-level")) == null ? void 0 : _a.className;
				const lv = (_b = c2 == null ? void 0 : c2.match(/level-([1-6])/)) == null ? void 0 : _b[1];
		...
		sub: {
			...
			content: (comment) => {
				var _a, _b, _c, _d;
				return (_d = (_c = (_b = (_a = comment.querySelector(".reply-content")) == null ? void 0 : _a.textContent) == null ? void 0 : _b.trim()) == null ? void 0 : _c.replace(/^回复\s?@[^@\s]+\s?:/, "")) == null ? void 0 : _d.replace(/@[^@\s]+/g, " ").trim();
			},
			callUser: (comment) => {
				var _a, _b, _c, _d, _e;
				return (_e = (_d = (_c = (_b = (_a = comment.querySelector(".reply-content")) == null ? void 0 : _a.textContent) == null ? void 0 : _b.trim()) == null ? void 0 : _c.replace(/^回复\s?@[^@\s]+\s?:/, "")) == null ? void 0 : _d.match(/@[^@\s]+/)) == null ? void 0 : _e[0].replace("@", "").trim();
			},
			callUserOnly: (comment) => {
				var _a, _b, _c, _d;
				return ((_d = (_c = (_b = (_a = comment.querySelector(".reply-content")) == null ? void 0 : _a.textContent) == null ? void 0 : _b.trim()) == null ? void 0 : _c.replace(/^回复\s?@[^@\s]+\s?:/, "")) == null ? void 0 : _d.replace(/@[^@\s]+/g, " ").trim()) === "";
			...
			level: (comment) => {
				var _a, _b;
				const c2 = (_a = comment.querySelector(".sub-user-level")) == null ? void 0 : _a.className;
				const lv = (_b = c2 == null ? void 0 : c2.match(/level-([1-6])/)) == null ? void 0 : _b[1];
				return lv ? parseInt(lv) : void 0;
			},
			isUp: (comment) => {
	...
	class CommentFilterSpace {
		...
		init() {
			...
			this.commentUsernameFilter.setParam(BiliCleanerStorage.get(`BILICLEANER_${GM_KEYS$8.black.username.valueKey}`, []));
			this.commentContentFilter.setParam(BiliCleanerStorage.get(`BILICLEANER_${GM_KEYS$8.black.content.valueKey}`, []));
			this.commentLevelFilter.setParam(BiliCleanerStorage.get(`BILICLEANER_${GM_KEYS$8.black.level.valueKey}`, 0));
			this.commentBotFilter.setParam(bots);
			this.commentCallBotFilter.setParam(bots);
			this.commentCallUserFilter.setParam([`/./`]);
		}
		async check(mode) {
			...
			const rootComments = Array.from(this.target.querySelectorAll(rootSelector));
			const subComments = Array.from(this.target.querySelectorAll(subSelector));
			if(settings.enableDebugFilter) {
				rootComments.forEach((v2) => {
					debugFilter(
						[`CommentFilterSpace rootComments`, `username: ${selectorFns$8.root.username(v2)}`, `content: ${selectorFns$8.root.content(v2)}`, `callUser: ${selectorFns$8.root.callUser(v2)}`, `callUserOnly: ${selectorFns$8.root.callUserOnly(v2)}`, `level: ${selectorFns$8.root.level(v2)}`, `isUp: ${selectorFns$8.root.isUp(v2)}`, `isPin: ${selectorFns$8.root.isPin(v2)}`, `isNote: ${selectorFns$8.root.isNote(v2)}`, `isLink: ${selectorFns$8.root.isLink(v2)}`].join("\n"));
				});
				subComments.forEach((v2) => {
					debugFilter(
						[`CommentFilterSpace subComments`, `username: ${selectorFns$8.sub.username(v2)}`, `content: ${selectorFns$8.sub.content(v2)}`, `callUser: ${selectorFns$8.sub.callUser(v2)}`, `callUserOnly: ${selectorFns$8.sub.callUserOnly(v2)}`, `level: ${selectorFns$8.sub.level(v2)}`, `isUp: ${selectorFns$8.sub.isUp(v2)}`, `isLink: ${selectorFns$8.sub.isLink(v2)}`].join("\n"));
				});
			}
			if(!rootComments.length && !subComments.length) {
				return;
			}
			if(isRootWhite$1 || revertAll) {
				rootComments.forEach((el) => showEle(el));
			}
			if(isSubWhite$1 || revertAll) {
				subComments.forEach((el) => showEle(el));
			}
			if(revertAll) {
				return;
			...
			if(!isRootWhite$1 && rootComments.length) {
				...
				this.commentCallBotFilter.isEnable && blackPairs.push([this.commentCallBotFilter, selectorFns$8.root.callUser]);
			...
			if(!isSubWhite$1 && subComments.length) {
				...
				this.commentCallBotFilter.isEnable && blackPairs.push([this.commentCallBotFilter, selectorFns$8.sub.callUser]);
			...
			const time = (performance.now() - timer).toFixed(1);
			debugFilter(`CommentFilterSpace hide ${rootBlackCnt} in ${rootComments.length} root, ${subBlackCnt} in ${subComments.length} sub, mode=${mode}, time=${time}`);
		}
		checkFull() {
		...
		observe() {
			waitForEle(document, "#app", (node) => {
				...
				if(ele) {
					debugFilter("CommentFilterSpace target appear");
					this.target = ele;
					this.checkFull();
					const commentObserver = new MutationObserver(() => {
						this.checkIncr();
					});
					commentObserver.observe(this.target, {
						childList: true,
						subtree: true
					});
				}
	...
	const mainFilter$8 = new CommentFilterSpace();
	const commentFilterSpaceEntry = async () => {
		mainFilter$8.init();
		mainFilter$8.observe();
	};
	const commentFilterSpaceGroups = [{
		name: "评论用户过滤",
		items: [{
			type: "switch",
			...
			editorDescription: ["每行一个用户名，保存时自动去重"],
			saveFn: async () => {
				mainFilter$8.commentUsernameFilter.setParam(BiliCleanerStorage.get(GM_KEYS$8.black.username.valueKey, []));
				mainFilter$8.checkFull();
			}
		}]
	}, {
	...
	const commentFilterSpaceHandler = (target) => {
		var _a, _b;
		if(!isPageSpace()) {
			return [];
		}
	...
	class CommentFilterVideo {
		...
		async checkRoot(mode) {
			...
			if(settings.enableDebugFilter) {
				rootComments.forEach((v2) => {
					debugFilter(
						[`CommentFilterVideo rootComments`, `username: ${selectorFns$7.root.username(v2)}`, `content: ${selectorFns$7.root.content(v2)}`, `callUser: ${selectorFns$7.root.callUser(v2)}`, `callUserOnly: ${selectorFns$7.root.callUserOnly(v2)}`, `level: ${selectorFns$7.root.level(v2)}`, `isUp: ${selectorFns$7.root.isUp(v2)}`, `isPin: ${selectorFns$7.root.isPin(v2)}`, `isNote: ${selectorFns$7.root.isNote(v2)}`, `isLink: ${selectorFns$7.root.isLink(v2)}`].join("\n"));
				});
			}
			...
			this.commentCallBotFilter.isEnable && blackPairs.push([this.commentCallBotFilter, selectorFns$7.root.callUser]);
			...
			const rootBlackCnt = await coreCheck(rootComments, true, blackPairs, whitePairs);
			const time = (performance.now() - timer).toFixed(1);
			debugFilter(`CommentFilterVideo hide ${rootBlackCnt} in ${rootComments.length} root comments, mode=${mode}, time=${time}`);
		}
		/**
		 * 检测二级评论
		 * @param mode full全量，incr增量
		...
		async checkSub(mode) {
			...
			if(settings.enableDebugFilter) {
				subComments.forEach((v2) => {
					debugFilter(
						[`CommentFilterVideo subComments`, `username: ${selectorFns$7.sub.username(v2)}`, `content: ${selectorFns$7.sub.content(v2)}`, `callUser: ${selectorFns$7.sub.callUser(v2)}`, `callUserOnly: ${selectorFns$7.sub.callUserOnly(v2)}`, `level: ${selectorFns$7.sub.level(v2)}`, `isUp: ${selectorFns$7.sub.isUp(v2)}`, `isLink: ${selectorFns$7.sub.isLink(v2)}`].join("\n"));
				});
			}
			...
			this.commentCallBotFilter.isEnable && blackPairs.push([this.commentCallBotFilter, selectorFns$7.sub.callUser]);
			...
			const subBlackCnt = await coreCheck(subComments, false, blackPairs, whitePairs);
			const time = (performance.now() - timer).toFixed(1);
			debugFilter(`CommentFilterVideo hide ${subBlackCnt} in ${subComments.length} sub comments, mode=${mode}, time=${time}`);
		}
		check(mode) {
			this.checkRoot(mode).then().catch((err) => {
	...
	const mainFilter$7 = new CommentFilterVideo();
	const commentFilterVideoEntry = async () => {
		mainFilter$7.init();
		mainFilter$7.observe();
	};
	const commentFilterVideoGroups = [{
		name: "评论用户过滤",
		items: [{
			type: "switch",
			...
			editorDescription: ["每行一个用户名，保存时自动去重"],
			saveFn: async () => {
				mainFilter$7.commentUsernameFilter.setParam(BiliCleanerStorage.get(GM_KEYS$7.black.username.valueKey, []));
				mainFilter$7.check("full");
			}
		}]
	}, {
	...
	const commentFilterVideoHandler = (target) => {
		var _a, _b;
	...
	class DynamicFilterDynamic {
		...
		async check(mode) {
			...
			let revertAll = false;
			if(!(this.dynUploaderFilter.isEnable || this.dynDurationFilter.isEnable || this.dynVideoTitleFilter.isEnable)) {
				revertAll = true;
			}
			const timer = performance.now();
			...
			if(settings.enableDebugFilter) {
				dyns.forEach((v2) => {
					debugFilter(
						[`DynamicFilterDynamic`, `uploader: ${selectorFns$6.uploader(v2)}`, `title: ${selectorFns$6.title(v2)}`, `duration: ${selectorFns$6.duration(v2)}`].join("\n"));
				});
			}
			const blackPairs = [];
	...
	class VideoFilterHomepage {
		...
		async check(mode) {
			...
			if(settings.enableDebugFilter) {
				videos.forEach((v2) => {
					debugFilter(
						[`VideoFilterHomepage`, `bvid: ${selectorFns$4.bvid(v2)}`, `duration: ${selectorFns$4.duration(v2)}`, `title: ${selectorFns$4.title(v2)}`, `uploader: ${selectorFns$4.uploader(v2)}`, `pubdate: ${selectorFns$4.pubdate(v2)}`, `isFollow: ${selectorFns$4.isFollow(v2)}`].join("\n"));
				});
			}
			const blackPairs = [];
		...
		setup(__props) {
			...
			return (_ctx, _cache) => {
				return show.value ? (e$1.openBlock(), e$1.createElementBlock("div", {
					key: 0,
					class: "fixed z-50 block cursor-pointer overflow-hidden rounded-md bg-white text-[15px] text-black shadow-lg shadow-black/20",
					style: e$1.normalizeStyle({
						left: pos.left + "px",
						top: pos.top + "px"
					})
...
https://www.bilibili.com${location.pathname}`;
	...
	const bangumiPlayerLayoutItems = [{
		...
		disableFn: async () => document.removeEventListener("wheel", disableAdjustVolume$1),
		enableFnRunAt: "document-end"
	}, {
		type: "switch",
...
#user-name a {color: #61666d !important;}`);
		...
		defaultEnable: true,
		enableFn: () => {
			ShadowInstance.addShadowStyle("bili-photoswipe", "video-page-reply-view-image-optimize", `#wrap:has(#thumb:empty) :is(#prev, #next) {display: none !important;}
#prev, #next {zoom: 1.3;}
...
.nav-user-center .user-con .item:has(.mini-favorite)`);
	...
	const homepagePluginItems = [{
		type: "switch",
		id: "homepage-hide-up-info-icon-bilibili-app-recommend",
		name: "隐藏 视频tag"
	}, {
		type: "switch",
		id: "homepage-hide-danmaku-count-bilibili-app-recommend",
		name: "隐藏 弹幕数"
	}, {
		type: "switch",
		id: "homepage-hide-agree-count-bilibili-app-recommend",
		name: "隐藏 点赞数"
	}];
	...
	const popularLayoutItems = [{
		...
		description: ["默认隐藏视频简介、标签", "对 综合热门/每周必看/入站必刷/排行榜 生效", '使用 5 列或 6 列布局时，建议开启 "隐藏 弹幕数"'],
		options: [{
...
https://www.bilibili.com/video/${avbv}` : `https://www.bilibili.com/video/${avbv}`;
	...
	const watchlaterStyle = "html[watchlater-page-layout-2-column] .list-box>span{display:grid!important;grid-template-columns:repeat(2,calc(50% - 10px))!important;gap:16px 20px!important}html[watchlater-page-layout-2-column] .watch-later-list{margin-bottom:50px!important}html[watchlater-page-layout-2-column] .watch-later-list header{margin:8px 0 16px!important}html[watchlater-page-layout-2-column] .av-item{width:unset!important;margin:0!important;border-radius:8px!important;padding:8px 8px 8px 32px!important}html[watchlater-page-layout-2-column] .av-item:hover{box-shadow:0 0 8px #0003!important;transition:box-shadow .1s linear}html[watchlater-page-layout-2-column] .av-item .av-about{display:flex!important;flex-direction:column!important;border-bottom:none!important}html[watchlater-page-layout-2-column] .av-item .av-pic .branch,html[watchlater-page-layout-2-column] .av-item .av-pic .corner{font-size:13px!important}html[watchlater-page-layout-2-column] .av-item .av-about .t{font-size:15px!important;font-weight:500!important;width:unset!important;overflow:unset!important;text-wrap:wrap!important;line-height:1.4em!important}html[watchlater-page-layout-2-column] .av-item .av-about .info .state{margin-left:auto!important}html[watchlater-page-layout-2-column] .av-item .av-about .info .state .looked{font-size:14px!important;margin-right:unset!important}html[watchlater-page-layout-2-column] .info.clearfix{margin-top:auto!important;display:flex!important;flex-direction:initial!important;align-items:center!important}html[watchlater-page-layout-2-column] .info.clearfix .user{display:flex!important;align-items:center!important}html[watchlater-page-layout-2-column] .info.clearfix .user span:last-child{margin:0 0 0 6px!important;font-size:14px!important;float:unset!important}";
	const rules = [{
		name: "首页",
	...
	const _sfc_main$2 = /* @__PURE__ */ e$1.defineComponent({
		...
		setup(__props) {
			...
			e$1.watch(btnPos.value, (newBtnPosition) => {
			...
			return (_ctx, _cache) => {
				return e$1.unref(sideBtnStore).isShow ? (e$1.openBlock(), e$1.createElementBlock("div", {
					...
					class: e$1.normalizeClass(["group fixed flex flex-col justify-end text-black text-opacity-50 hover:text-opacity-100", {
