...
if(url.includes("/mtop.idle.user.page.my.adapter/")) {
		...
		for(let items of obj.data.container.sections) {
			if(items?.template?.name === "my_fy25_user_info") {
				...
				if(items?.item?.tool?.exContent?.tools?.length > 0) {
					items.item.tool.exContent.tools = items.item.tool.exContent.tools.map((i) => i.filter((ii) => ![14]?.includes(ii?.exContent?.toolId)));
				}
	...
	if(obj?.data?.sections?.length > 0) {
		obj.data.sections = obj.data.sections.filter(
			(i) => !["fish_home_advertise_card_d4", "fish_home_content_card", "fish_home_feeds_commodity_card_2", "fish_home_feeds_pager_banner"]?.includes(i?.template?.name));
	}
	if(obj?.data?.widgetReturnDO?.widgets?.length > 0) {
		...
		if(widget?.widgetDO?.channelDOList?.length > 0) {
			widget.widgetDO.channelDOList = widget.widgetDO.channelDOList.filter((i) => sortLists?.includes(i?.title)).sort((a, b) => sortLists.indexOf(a?.title) - sortLists.indexOf(b?.title));
		}
	}
	...
	if(obj?.data?.resultList?.length > 0) {
		obj.data.resultList = obj.data.resultList.filter(
			(i) => !(i?.data?.template?.name === "idlefish_seafood_vote" || // 搜索结果 投票
